...
public class Portal {
	...
	private Portal(Blox topLeft, int modX, int modZ,
			...
			boolean hidden, boolean alwaysOn, boolean priv, boolean free, boolean backwards, boolean show, boolean noNetwork, boolean random, boolean bungee, boolean donor) {
	...
	public static Portal createPortal(SignChangeEvent event, Player player) {
		...
		boolean bungee = (options.indexOf('u') != -1);
                boolean donor = (options.indexOf('z') != -1);
		...
		Blox button = null;
		Portal portal = null;
		portal = new Portal(topleft, modX, modZ, rotX, id, button, destName, name, false, network, gate, player.getName(), hidden, alwaysOn, priv, free, backwards, show, noNetwork, random, bungee, donor);
			...
			try {
				...
				while (scanner.hasNextLine()) {
					...
					boolean bungee = (split.length > 20) ? split[20].equalsIgnoreCase("true") : false;
                                        boolean donor = (split.length > 21) ? split[21].equalsIgnoreCase("true") : false;

					Portal portal = new Portal(topLeft, modX, modZ, rotX, sign, button, dest, name, false, network, gate, owner, hidden, alwaysOn, priv, free, backwards, show, noNetwork, random, bungee, donor);
					portal.register();
					portal.close(true);
				}
				scanner.close();
